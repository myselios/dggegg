...
| Phase | 작업 | 설명 |
|-------|------|------|
| 0-1 | 프로젝트 기반 세팅 | Supabase 연동, shadcn/ui, 패스워드 게이트, 레이아웃 |
| 0-2 | DB 스키마 | Supabase 마이그레이션, 타입 생성 |
| 0-3 | 학생 모듈 | 카드 보드 + 상세 페이지 + CRUD |
| 0-4 | 스케줄 모듈 | 3주 캘린더 + 드래그앤드롭 + 반복 수업 |
| 0-5 | 수업 기록 | 슬라이드 패널 + 템플릿 + 성적 연결 |
| 0-6 | 상담 로그 | CRUD + 검색 + 태그 |
| 0-7 | Dashboard | 오늘 수업 + 미니 캘린더 + 최근 상담 |
